...
class AcronymAllowlistGate(PolicyGate):
    ...
    def find_violations_in_text(
        ...
        *,
        ...
        prose = AcronymAllowlistGate.strip_markdown_code(text)
        violations: list[AcronymViolation] = []
        for line_no, line in enumerate(prose.splitlines(), start=1):
            for token in AcronymAllowlistGate.find_violations_in_line(
                line, allowlisted=allowlisted
            ):
    ...
    def scan_paths(
        *,
        ...
        allowlisted = AcronymAllowlistGate.load_allowlist(allowlist_path)
        violations: list[AcronymViolation] = []
        scoped_files = scope_paths_from_env()
        if scoped_files:
            file_paths = [repo_root / rel for rel in scoped_files]
        else:
            file_paths = AcronymAllowlistGate.iter_markdown_files(scan_roots, repo_root)
        for file_path in file_paths:
            ...
            violations.extend(
                AcronymAllowlistGate.find_violations_in_text(
                    text, path=rel, allowlisted=allowlisted
                )
            )
        return violations
    ...
    def collect_findings(
        ...
        *,
        ...
        scan_roots, allowlist_path = self.settings_from_config(dict(config))
        violations = self.scan_paths(
            repo_root=root,
            allowlist_path=self.require_allowlist_path(root, allowlist_path),
            scan_roots=scan_roots,
            ignores=ignores,
        )
        return self.findings_from_violations(violations)
