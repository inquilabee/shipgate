...
class GitHubFormatter(BaseFormatter):
    def render_lines(self, report: RunReport) -> list[str]:
        ...
        for check, finding in self.iter_findings(report):
            if finding.rule_id == TOOL_EXIT_RULE:
                title = self.escape(f"{check.check_id}/{finding.rule_id}")
                message = self.escape(finding.message)
                lines.append(f"::error title={title}::{message}")
            else:
                lines.append(self.format_annotation(check.check_id, finding))
        return lines
    ...
    def format_annotation(check_id: str, finding: Finding) -> str:
        title = GitHubFormatter.escape(f"{check_id}/{finding.rule_id}")
        message = GitHubFormatter.escape(finding.message)
