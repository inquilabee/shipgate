...
def test_planning_incremental_does_not_import_runtime():
    src = Path("src/shipgate/planning/incremental.py").read_text(encoding="utf-8")
    tree = ast.parse(src)
...
def test_planning_check_planner_does_not_import_runtime_session():
    src = Path("src/shipgate/planning/check_planner.py").read_text(encoding="utf-8")
    tree = ast.parse(src)
    for node in ast.walk(tree):
        if (
            isinstance(node, ast.ImportFrom)
