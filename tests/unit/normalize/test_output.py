...
from shipgate.runtime.session.check_runner import CheckRunner, OutputFileSnapshot
...
def test_execute_check_ignores_leftover_json_not_written_this_run(tmp_path: Path):
    tool = CatalogLoader.load().get_tool("bandit.scan")
    output_path = tmp_path / "bandit.scan.json"
    output_path.write_text('{"results": ["STALE"]}', encoding="utf-8")
    resolved = ResolvedRequest(
    ...
    runner = CheckRunner(
        ...
        executor=executor,
        executor_is_default=False,
    )
    report = runner.execute_check(resolved, "run-1", no_cache=True)
    serialized = str(report.findings) + str(report.extra)
